...
export default function VaultApp({ T, dark, setDark, user, isGuest, storage, onLogout, onLogin }) {
  ...
  const DEMO_VERSION = 3; // bump when DEMO data changes
  useEffect(() => {
    let cancelled = false;
    storage.getRepos().then(data => {
      ...
      // Auto-repair missing readmes in demo data
      let finalData = data;
      if (isGuest && data?.length > 0) {
        let repaired = false;
        finalData = data.map(r => {
          if (["1", "2", "3", "4", "5", "6"].includes(r.id) && !r.readme) {
            const demoRes = DEMO.find(d => d.id === r.id);
            if (demoRes && demoRes.readme) {
              repaired = true;
              const fixedRepo = { ...r, readme: demoRes.readme };
              storage.updateRepo(r.id, fixedRepo);
              return fixedRepo;
            }
          }
          return r;
        });
        if (repaired) data = finalData;
      }

      // Re-seed if completely empty or severely outdated format
