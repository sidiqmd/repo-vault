  ...
  return (
    <div style={{ borderBottom: `1px solid ${T.bdrLt}`, background: T.toolbarBg }}>
      <div style={{ maxWidth: 1360, margin: "0 auto", padding: "6px 16px", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
        <select value={fSt} onChange={e => setFSt(e.target.value)} style={selS}>
          <option value="all">Status</option>
          {STATUSES.map(s => <option key={s.id} value={s.id}>{s.label}</option>)}
        </select>
        <select value={fCat} onChange={e => setFCat(e.target.value)} style={selS}>
          <option value="all">Category</option>
          {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.label}</option>)}
        </select>
        <select value={sort} onChange={e => setSort(e.target.value)} style={selS}>
